var oSL = {Regform:function () {
}};
var $Y = YAHOO, $D = YAHOO.util.Dom, $C = $D.getElementsByClassName, $CONN = YAHOO.util.Connect, $ = YAHOO.util.Dom.get, $LANG = YAHOO.lang, $COOKIE = YAHOO.util.Cookie, $J = YAHOO.lang.JSON, $W = YAHOO.widget, $L = YAHOO.log;
LampcmsException = function (message, exceptionName) {
    this.message = message;
    this.name = exceptionName || "LampcmsException";
};
oAjaxObject = {handleSuccess:function (o) {
    var eLastDiv, json, sDoc, sTpl, errDiv, strMessage = '', eLogin = $("loginHead"), strContentType = $LANG.trim(o.getResponseHeader["Content-Type"]);
...
oSL = {toString:function () {
    ...
    var tzo = this.getTZO();
    $L('117 tzo: ' + tzo);
    $COOKIE.set("tzo", tzo, {path:"/"});
}, oCallback:{success:oAjaxObject.handleSuccess, failure:oAjaxObject.handleFailure, scope:oAjaxObject}, fAddIcon:function (s, b) {
    ...
    var myChange, curBg, myChangeBack, element = (typeof el === 'string') ? $(el) : el, sToColor = (sToColor && typeof sToColor === 'string') ? sToColor : '#FF0000', sFromColor = (sFromColor && typeof sFromColor === 'string') ? sFromColor : '#FFFFFF';
    $L('element is: ' + element);
    if (element) {
        curBg = $D.getStyle(element, 'background-color');
        $D.setStyle(element, 'background-color', sFromColor);
        myChange = new YAHOO.util.ColorAnim(element, {backgroundColor:{to:sToColor}});
    ...
    switch (true) {
        ...
        case json.hasOwnProperty('errors'):
            ...
            this.fColorChange('qfmessage', '#FF0000', '#FFFFFF');
            break;
    }
...
oSL.tweet = (function () {
    ...
    return{getInstance:function () {
        ...
        if (!oDialog) {
            ...
            oDialog = new $W.Dialog("dialog1", {width:"30em", fixedcenter:true, visible:false, constraintoviewport:true, buttons:[
            ...
            oDialog.callback = {success:function (o) {
                alert('Tweet sent');
            }, failure:function (o) {
...
oSL.Regform = (function () {
    var errDiv, oDialog, aDialogs = {}, oPrompt, handleSubmit = function () {
        this.submit();
        ...
        var i, errDiv = $('form_error'), aInputs, message = oError.exception, oRegform = oSL.Regform.getInstance(), myForm = oRegform.form;
        ...
        {text:"<-- Return to page", handler:function () {
            alert('this is ' + this);
        }, isDefault:true},
        {text:"Go to Profile editor -->", handler:function () {
            alert('go to profile');
        }}
    ...
    return{getInstance:function () {
        ...
        if (!oDialog) {
            ...
            $L('cp 17', 'warn');
            oDialog = new $W.Dialog("regdiv", {width:"50em", fixedcenter:"contained", visible:false, constraintoviewport:false, hideaftersubmit:false, draggable:true, close:false, modal:true, buttons:[
                {text:"Create Your Account", handler:handleSubmit, isDefault:true},
                {text:"Cancel", handler:handleCancel}
            ]});
            oDialog.callback = {success:handleSuccess, failure:handleFailure};
            oDialog.validate = function () {
                var at, checkEmail, tzo, message, aInputs = [], myForm = this.form, nd = new Date(), data = this.getData();
                ...
                setError({exception:message, fields:aInputs});
                return false;
            };
        ...
        if (!oPrompt) {
            $L('making prompt');
            oPrompt = new $W.SimpleDialog("simpledialog1", {width:"300px", fixedcenter:true, zindex:99, visible:false, draggable:false, close:true, modal:true, text:"Do you want to continue?", icon:$W.SimpleDialog.ICON_ALARM, constraintoviewport:true, buttons:[
                {text:"Continue registration", handler:handleContinue, isDefault:true},
                {text:"Exit registration", handler:handleExit}
            ], effect:[
                {effect:$W.ContainerEffect.FADE, duration:0.2}
            ]});
...
YUI({}).use('node', 'dump', 'event', 'escape', 'gallery-storage-lite', 'gallery-overlay-extras', 'dd-plugin', 'anim', 'transition', 'yui2-container', 'yui2-editor', 'yui2-resize', 'io-form', 'json', 'jsonp', 'imageloader', 'autocomplete', 'autocomplete-filters', 'autocomplete-highlighters', 'gallery-node-tokeninput', 'cookie', function (Y, result) {
    var YAHOO = Y.YUI2, TTT2 = Y.all('.ttt2'), ttB, ttB2, oMetas = {}, loader, getMeta, setMeta, getToken, ensureLogin, initTooltip, getEditedText, previewDiv, preview, MysubmitForm, showDeleteForm, showRetagForm, showShredForm, showCommentForm, addAdminControls, checkExtApi, showFlagForm, showCloseForm, codeButtons = {}, initAutoComplete, getAlerter, isModerator, isEditable, initFBSignup, Twitter, showEditComment, setToken, getReputation, isLoggedIn, getViewerId, oCTabs = {}, foldGroup, revealComments, dnd = false, res = Y.one('#body_preview'), write = function (str) {
        var d = new Date();
        str += ' :: ' + d.toTimeString();
        if (res) {
        ...
        if (input) {
            ...
            Y.one(input).plug(Y.Plugin.TokenInput, {delimiter:' '}).plug(Y.Plugin.AutoComplete, {resultListLocator:'ac', resultTextLocator:'tag', resultFilters:'charMatch', resultHighlighter:'charMatch', source:'/index.php?a=taghint&q={query}&ajaxid=1&callback={callback}'});
        }
        ...
        var uriParts, i = 0, uri = {}, uriPartNames = ["source", "protocol", "authority", "domain", "port", "path", "directoryPath", "fileName", "query", "anchor"];
        ...
        var myID, getID, path, a = parseUri(url), re = /(?:v=)([^&\?]*)(?:[&]*)/gi;
        path = a['path'];
        ...
        var url, apiURL = 'http://gdata.youtube.com/feeds/api/videos/{id}?v=2&alt=jsonc&callback=', handleJSONP, imgId, myinput1 = (Y.one("#embed_url")) ? Y.one("#embed_url").get('value') : null;
        handleJSONP = function (resp) {
            var _doc = editor._getDoc(), el, html, imgId, title = '', desc = '', err = 'Error returned from Youtube API. ', tpl = '<a href="http://youtu.be/{id}" class="ajax ytlink ttt" rev="{id}" title="{t}"><img src="http://i.ytimg.com/vi/{id}/default.jpg" alt="YouTube video" width="120px" height="90px"/></a>';
        ...
        var btn, win = new YAHOO.widget.EditorWindow('insertmedia', {width:'415px'});
        ...
        if (!loader) {
            loader = new Y.Overlay({centered:true, srcNode:"#loading", width:"100px", headerContent:"Loading...", bodyContent:"<img src='/images/loading-bar.gif'>", zIndex:1000});
        ...
        myAnim = new Y.Anim({node:div, to:{width:480, height:390}});
        ...
        FB.ui({method:'stream.publish', message:'I joined this site with Facebook Connect button. You should check it out too', attachment:{name:siteTitle, caption:caption, description:siteDescription, href:siteUrl}, action_links:[
            {text:siteTitle, href:siteUrl}
        ], user_message_prompt:'Invite your Facebook Friends to join this site'}, function (response) {
        });
    }, handleModalForm = function (e) {
        var request, cfg, form = e.currentTarget;
        e.halt();
        cfg = {method:'POST', form:{id:form, useDisabled:true}};
        oAlerter.hide();
        showLoading();
        ...
        var ancestor, id, rtype, restype, resID, fbappid, fbcookie, el = e.currentTarget;
        id = el.get('id');
        switch (true) {
            case el.test('.qpages'):
        ...
        var href, curTab, curTabId, sortby = el.get('id'), qid, qtype = Y.one("#qtypes"), eTab = el.ancestor("div").next("div.sortable") || Y.one(".sortable");
        ...
        var viewerDiv, title, controls, resID, ftype = 'q', follow = 'on', form, oLabels = {'q':'question', 'u':'user', 't':'tag'};
        ...
        cfg = {method:'POST', form:{id:form}};
        request = Y.io('/index.php', cfg);
        return;
    }, handleOver = function (e) {
        var sFlw = '<span class="icoc del">&nbsp;</span><span> Unfollow</span>', el = e.currentTarget;
        switch (true) {
        ...
        var sFlw = '<span class="icoc check">&nbsp;</span><span> Following</span>', el = e.currentTarget;
        switch (true) {
            case(el.test('.following')):
                el.removeClass('unfollow');
        ...
        if (data.vote && data.vote.hasOwnProperty('v') && data.vote.rid) {
            ...
            if (data.redirect || data.answer) {
                ...
                removeTags();
                if (data.redirect) {
                    getAlerter('<h3>Success</h3>').set("bodyContent", 'Item saved! Redirecting to <br><a href="' + data.redirect + '">' + data.redirect + '</a>').show();
    ...
    var getYTbutton = function () {
        ...
        if ('1' == getMeta('btn_yt')) {
            ret = {type:'push', label:'Insert YouTube Video', value:'insertmedia', id:'btn_youtube'};
        }
        return ret;
    };
    var getCodeButton = function () {
        var ret = {type:'separator'};
        if (typeof dp !== 'undefined') {
            ret = {group:'sourcecode', label:'Code style', buttons:[
                {type:'select', label:'Select', value:'codestyle', disabled:true, menu:[
                    {text:'None', value:'nocode', checked:true},
    ...
    var makeEditor = function () {
        var codeButtons, btnSeparator = {type:'separator'};
        if (Y.one("#id_qbody") && Y.all('.com_hand').isEmpty()) {
            codeButtons = getCodeButton();
            editor = new YAHOO.widget.Editor('id_qbody', {dompath:false, width:'650px', height:'180px', autoHeight:!(Y.UA.ie > 0), extracss:'pre { margin-left: 10px; margin-right: 10px; padding: 2px; background-color: #EEE; } a.ytplay{position: absolute; display: block; height: 44px; width: 44px; top: 23px; left: 38px;} .yui-media { height: 90px; width: 120px; border: 1px solid black; background-color: #f2f2f2; background-image: url( "/images/media.gif" ); background-position: 45% 45%; background-repeat: no-repeat; }  .ytvideo {border: 1px solid black; cursor: pointer; position: relative; clear: both; margin-bottom: 5px;}', toolbar:{buttons:[
                {group:'saveclear', label:'Save / New', buttons:[
                    {type:'push', label:'Save Draft', value:'save'},
                    {type:'push', label:'New Document', value:'clear'}
                ]},
                ...
                btnSeparator,
                {group:'blockquote', label:'Quote', buttons:[
                ...
                codeButtons,
                btnSeparator,
                {group:'insertitem', label:'Link / Image', buttons:[
                    {type:'push', label:'HTML Link CTRL + SHIFT + L', value:'createlink', disabled:true},
                    {type:'push', label:'Insert Image', value:'insertimage', disabled:false},
                    getYTbutton()
                ]},
                btnSeparator,
            ...
            editor.on('toolbarLoaded', function () {
                this.on('afterNodeChange', function (o) {
                    var ytbtn = this.toolbar.getButtonByValue('insertmedia'), btn = this.toolbar.getButtonByValue('codestyle');
                    if (btn) {
                ...
                editor.toolbar.on('codestyleClick', function (ev) {
                    var escaped, html, sel = this._getSelection(), newEl, el = editor._getSelectedElement(), codetype = ev.button.value.toLowerCase();
            ...
            editor.on('windowRender', function () {
                ...
                editor._windows.insertmedia = {body:body};
            });
            if (!Y.one('#iedit')) {
            ...
            preview = function () {
                var h, sh, previewDiv, pd = Y.one('#tmp_preview');
    ...
    var deleteComment = function (resID) {
        ...
        if (confirm('Really delete this comment?')) {
            ...
            if (comment) {
                ...
                cfg = {method:'POST', form:{id:f, useDisabled:true}};
                request = Y.io('/index.php', cfg);
    ...
    getAlerter = function (header) {
        if (!oAlerter) {
            oAlerter = new Y.Overlay({srcNode:'#fbOverlay', width:'500px', height:'300px', zIndex:100, centered:true, constrain:true, render:true, visible:false, plugins:[
    ...
    Twitter = {popupWindow:null, oInterval:null, startDance:function (url, w, h) {
        showLoading();
    ...
    initTooltip = function () {
        ...
        if (TTT && TTT.size() > 0) {
            ...
            ttB = new YAHOO.widget.Tooltip("ttB", {context:TTT._nodes, autodismissdelay:5500, width:'300px', hidedelay:350, xyoffset:[-10, -45], effect:{effect:YAHOO.widget.ContainerEffect.FADE, duration:0.20}});
        }
    };
    if (TTT2 && TTT2.size() > 0) {
        ttB2 = new YAHOO.widget.Tooltip("ttB2", {context:TTT2._nodes, autodismissdelay:5500, hidedelay:350, xyoffset:[-10, -45], effect:{effect:YAHOO.widget.ContainerEffect.FADE, duration:0.20}});
    }
    initAutoComplete = function () {
        ...
        if (isearch) {
            isearch.plug(Y.Plugin.AutoComplete, {minQueryLength:3, resultHighlighter:'wordMatch', resultListLocator:'ac', resultTextLocator:'title', source:'/index.php?a=titlehint&q={query}&ajaxid=1&callback={callback}'});
        }
        if (id_title) {
            id_title.plug(Y.Plugin.AutoComplete, {minQueryLength:3, resultHighlighter:'wordMatch', resultListLocator:'ac', resultTextLocator:'title', source:'/index.php?a=titlehint&q={query}&ajaxid=1&callback={callback}', resultFormatter:function (query, results) {
                return Y.Array.map(results, function (result) {
                    var tpl = '<a href="/q{_id}/{url}">{t}</a><br>{intro}<br><div class="c6">Asked {hts} (<span class="c_{status}">{i_ans} answer{ans_s})</span></div>';
                    var raw = result.raw;
                    return Y.Lang.sub(tpl, {_id:raw._id, url:raw.url, intro:raw.intro, t:result.highlighted, hts:raw.hts, i_ans:raw.i_ans, ans_s:raw.ans_s, status:raw.status});
                });
            }});
    ...
    foldGroup = new Y.ImgLoadGroup({foldDistance:2});
