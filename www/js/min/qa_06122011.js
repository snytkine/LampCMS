...
YUI({}).use('node', 'dump', 'event', 'escape', 'gallery-storage-lite', 'gallery-overlay-extras', 'dd-plugin', 'anim', 'transition', 'yui2-container', 'yui2-editor', 'yui2-resize', 'yui2-animation', 'io-form', 'json', 'jsonp', 'imageloader', 'autocomplete', 'autocomplete-filters', 'autocomplete-highlighters', 'gallery-node-tokeninput', 'cookie', function (Y, result) {
    var YAHOO = Y.YUI2, TTT2 = Y.all('.ttt2'), ttB, ttB2, oMetas = {}, loader, getMeta, setMeta, getToken, ensureLogin, initTooltip, getEditedText, previewDiv, preview, MysubmitForm, showDeleteForm, showRetagForm, showShredForm, showCommentForm, showCommentReplyForm, addAdminControls, checkExtApi, showFlagForm, showCloseForm, codeButtons = {}, initAutoComplete, getAlerter, isModerator, isEditable, initFBSignup, Twitter, showEditComment, setToken, getReputation, isLoggedIn, getViewerId, oCTabs = {}, foldGroup, revealComments, dnd = false, res = Y.one('#body_preview'), write = function (str) {
        var d = new Date();
        str += ' :: ' + d.toTimeString();
        if (res) {
        ...
        + '{commentTip}'
        + '</table>'
        + '</form></div>', saveToStorage = function () {
        ...
        var ancestor, id, res, rtype, restype, resID, fbappid, fbcookie, el = e.currentTarget, target = e.target;
        ...
        var viewerDiv, title, controls, id, resID, ftype = 'q', follow = 'on', form, oLabels = {'q':'question', 'u':'user', 't':'tag'};
    ...
    showCommentForm = function (el) {
        ...
        if (ensureLogin()) {
            if (('1' === getMeta('comment')) || (getMeta('asker_id') == vid) || (rep > minrep) || el.test('.uid-' + vid)) {
                ...
                if (!form) {
                    form = Y.Lang.sub(tplComform, {extraClass:'', divID:resID, resID:resID, formID:resID, token:getToken(), parentID:'0', comment:'comment', commentTip:commentTip});
                    ...
                    if (form._isHidden()) {
                        ...
                        form.hide('fadeOut', null, function (o) {
                            Y.log('3280 fadeout o is: ' + o + ' this is: ' + this, 'warn');
                        });
                    }
                }
            } else {
    ...
    showCommentReplyForm = function (el) {
        ...
        if (ensureLogin()) {
            if (('1' === getMeta('comment')) || (getMeta('asker_id') == vid) || (rep > minrep) || el.test('.uid-' + vid)) {
                ...
                if (!form) {
                    form = Y.Lang.sub(tplComform, {extraClass:' com_wrap', divID:parentID, resID:resID, formID:parentID, token:getToken(), comment:'reply', commentTip:commentTip, parentID:parentID});
    ...
    Y.one('body').delegate("hover", handleOver, handleOut, '.mo');
